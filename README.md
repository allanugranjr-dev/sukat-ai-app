...
For a deployed environment, configure a real provider. If provider settings are missing there, the function marks the scan `failed` with an actionable configuration message instead of leaving it in a queue that cannot advance. When a provider is configured, the function creates short-lived signed URLs for the private views, sends the scan metadata and URLs, validates the response, writes measurements and an optional body-model asset, and sets the scan to `ready_for_review`. Invalid or failed provider responses set `failed` with a reason.
