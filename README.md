...
`process-scan` requires all three view types. An explicit `RECONSTRUCTION_PROVIDER=local` setting—or an otherwise unconfigured request—uses a deterministic simulator. It writes clearly labeled demo measurements derived from the height reference and attaches the reimagined 3D model reference (`public/media/3d-body-scan-reference-v2.png`) for the interactive 3D viewer, then moves the scan to `ready_for_review`. These values are not a personalized reconstruction and must be checked by a dressmaker before tailoring.
