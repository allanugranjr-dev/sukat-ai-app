...
/* Reviewed branch: preserve current implementation. */
...
/* Incoming alternative omitted after reconciliation:
  const uploadInputId = useId();
  return <div className="capture-layout"><Panel className="capture-stage-card"><div className="capture-stage-heading"><div><p className="eyebrow">STEP 03 · CAPTURE</p><h2>{current?.label ?? "Front"} view</h2><p>Keep your full body in frame and follow the on-screen guide.</p></div><Badge tone={current?.captured ? "success" : "teal"} dot>{current?.captured ? "UPLOADED" : "READY"}</Badge></div><div className={cn("capture-stage", currentUrl && "has-capture")}>{cameraOn && <video ref={videoRef} autoPlay muted playsInline className="capture-video" aria-label={`${current?.label ?? "Current"} camera preview`} />}{!cameraOn && currentUrl && <img className="capture-preview" src={currentUrl} alt={`${current?.label} scan view`} />}{!cameraOn && !currentUrl && <div className="capture-empty"><span><Icon name="camera" size={31} /></span><strong>Camera or upload</strong><small>Your selected view will appear here after a successful upload.</small></div>}{cameraOn && <div className="capture-guide"><span /><span /><span /></div>}</div><div className="capture-controls"><div className="capture-progress"><p className="eyebrow">VIEWS</p><div>{captures.map((slot, index) => <button type="button" key={slot.key} className={cn(index === captureIndex && "current", slot.captured && "done")} onClick={() => setCaptureIndex(index)}><i>{slot.captured ? <Icon name="check" size={12} /> : index + 1}</i><small>{slot.label}</small></button>)}</div></div><div className="capture-actions">{cameraOn ? <><Button variant="secondary" onClick={onStopCamera} icon="x">Stop camera</Button><Button onClick={onCapture} disabled={uploading} icon="camera">{uploading ? "Uploading…" : "Capture frame"}</Button></> : <><input id={uploadInputId} className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" onChange={onChooseFile} /><label className="button button-secondary" htmlFor={uploadInputId}><Icon name="upload" size={15} /> {uploading ? "Uploading…" : "Upload image"}</label><Button variant="ghost" onClick={onStartCamera} disabled={uploading} icon="camera">Use camera</Button></>}</div></div></Panel><div className="capture-side"><Panel className="capture-next"><p className="eyebrow">NEXT STEP</p><h3>{captures.every((slot) => slot.captured) ? "All three views are ready." : `Add the ${captures.find((slot) => !slot.captured)?.label.toLowerCase() ?? "next"} view.`}</h3><p>{captures.every((slot) => slot.captured) ? "Submit the scan to the processing queue when you are satisfied with the uploads." : "You can move between views at any time and replace an uploaded image."}</p><Button onClick={onContinue} disabled={busy || uploading || captures.some((slot) => !slot.captured)} icon="arrow-right">{busy ? "Submitting…" : "Submit scan"}</Button></Panel><Panel className="capture-quality"><p className="eyebrow">UPLOAD REQUIREMENTS</p><div className="quality-list"><span><Icon name="check" size={14} /> JPG, PNG, or WebP</span><span><Icon name="check" size={14} /> Maximum 10 MB each</span><span><Icon name="check" size={14} /> Private signed access</span></div><div className="uploaded-list">{captures.map((slot, index) => <div key={slot.key}><span className={slot.captured ? "uploaded" : "not-uploaded"}><Icon name={slot.captured ? "check" : "clock"} size={12} /></span><span><strong>{slot.label}</strong><small>{slot.captured ? "Uploaded" : "Waiting"}</small></span>{slot.captured && <button type="button" className="icon-button" aria-label={`Remove ${slot.label} view`} onClick={() => void onRemove(index)}><Icon name="x" size={14} /></button>}</div>)}</div></Panel></div></div>;
*/
}
...
/* Incoming alternative omitted after reconciliation:
  ...
  addMeasuredLimbGuide(three, guides, rightShoulder.clone().lerp(rightElbow, 0.5), rightShoulder, rightElbow, bicepRadius, 0x74d96e);
   ...
   addMeasuredLimbGuide(three, guides, leftWrist, leftWrist, leftWrist.clone().add(new three.Vector3(0, -height * 0.08, 0)), wristRadius, 0x9e9cff);
   addMeasuredLimbGuide(three, guides, rightWrist, rightWrist, rightWrist.clone().add(new three.Vector3(0, -height * 0.08, 0)), wristRadius, 0x9e9cff);
   ...
   addMeasuredLimbGuide(three, guides, leftAnkle, leftAnkle, leftAnkle.clone().add(new three.Vector3(0, -height * 0.05, 0)), ankleRadius, 0x72e56f);
   addMeasuredLimbGuide(three, guides, rightAnkle, rightAnkle, rightAnkle.clone().add(new three.Vector3(0, -height * 0.05, 0)), ankleRadius, 0x72e56f);
*/
...
*/
/* Incoming alternative omitted after reconciliation:
   return <div className="model-3d-viewer"><div className="model-3d-stage" aria-busy={viewerState === "loading"}><canvas ref={canvasRef} tabIndex={0} aria-label="Interactive measurement-driven 3D body model" aria-describedby={instructionsId} onKeyDown={handleCanvasKeyDown} /><figure className="model-3d-reference"><img src={referenceImage} alt="" /><figcaption>Reference visual</figcaption></figure><div className="model-3d-scale" aria-label="Model scale summary"><strong>MEASURED PROPORTIONS</strong><span>{modelHeightLabel}</span><small>{measurements.length} result values mapped to the body</small></div>{viewerState === "loading" && <div className="model-3d-loading" role="status" aria-live="polite"><span className="model-3d-spinner" />Preparing measured body model…</div>}{viewerState === "fallback" && <div className="model-3d-fallback" role="status" aria-live="polite"><img src={referenceImage} alt="Reference visualization for the body measurement result" /><span>Interactive 3D is unavailable on this device. Showing the reference visual.</span></div>}<span className="model-preview-badge"><Icon name="scan" size={13} /> Measured body · interactive 3D</span></div><div className="model-3d-controls" aria-label="3D model controls"><button type="button" className={autoRotate ? "active" : ""} onClick={() => setAutoRotate((value) => !value)} disabled={viewerState !== "ready" || reducedMotion} aria-pressed={autoRotate}>{autoRotate ? "Pause rotation" : "Auto rotate"}<Icon name="rotate" size={14} /></button><button type="button" onClick={() => { controlsRef.current?.reset(); renderRequestRef.current?.(); }} disabled={viewerState !== "ready"}><Icon name="refresh" size={14} />Reset view</button><button type="button" onClick={zoomIn} disabled={viewerState !== "ready"}><Icon name="zoom-in" size={14} />Zoom in</button><button type="button" onClick={zoomOut} disabled={viewerState !== "ready"}><Icon name="zoom-in" size={14} />Zoom out</button><button type="button" className={showGuides ? "active" : ""} onClick={() => setShowGuides((value) => !value)} disabled={viewerState !== "ready"} aria-pressed={showGuides}><Icon name="ruler" size={14} />{showGuides ? "Hide guides" : "Show guides"}</button></div><p className="model-3d-hint" id={instructionsId}><Icon name="rotate" size={13} /> Drag to rotate · scroll or pinch to zoom · focus the model and use arrow keys, +/−, or Home</p>{reducedMotion && <p className="model-3d-motion-note" role="status">Auto-rotation is off because reduced motion is enabled.</p>}</div>;
*/
}
...
/* Incoming alternative omitted after reconciliation:
  ...
  return <div className="model-ready" role="status" aria-live="polite"><span className="model-empty-icon"><Icon name="expand" size={27} /></span><h3>{assetUrl ? "Body model asset available" : "Opening private model asset…"}</h3><p>The provider returned a model asset for authorized review.</p>{assetUrl && <a className="button button-secondary" href={assetUrl} target="_blank" rel="noreferrer" aria-label="Open model asset in a new tab">Open model asset <Icon name="external" size={15} /></a>}</div>;
*/
}
