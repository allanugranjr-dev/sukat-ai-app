...
function isLiveMeasurementsProvider(value: string | null): boolean {
  return ["live-measurements-api", "live-measurements", "javtahir-live-measurements", "javtahir"].includes((value ?? "").toLowerCase());
}
...
function AppShell({ profile, page, onNavigate, onSignOut, children }: { profile: Profile; page: Page; onNavigate: (page: string) => void; onSignOut: () => void; children: ReactNode }) {
  ...
  const go = (next: string) => { onNavigate(next); setMobileOpen(false); setMoreOpen(false); };
...
function ScanProcessing({ scanId, initialMessage, onBack, onResults }: { scanId: string; initialMessage: string; onBack: () => void; onResults: () => void }) {
  ...
  const progressLabel = progressLevel === "blocked" ? "Processing needs your attention" : progressLevel === "processing" ? "Processing is in progress" : "Waiting for processing to start";
  const statusDetail = unavailable
    ? "The processing service is unavailable. Your photos are safe and can be retried."
    : status === "processing"
      ? "The service is checking your uploaded views. We’ll move you to review when a valid result is ready."
      : "Your uploaded views are stored securely while the service gets ready. We’ll keep checking for a result.";
...
function ScanResults({ scanId, onRecapture, onDashboard }: { scanId: string; onRecapture: () => void; onDashboard: () => void }) {
  ...
  const liveMeasurementsProvider = isLiveMeasurementsProvider(bundle?.scan.processing_provider ?? null);
...
function measurementGuideKey(key: string): string | null {
  const normalized = normalizeModelMeasurementKey(key);
  const sideSuffix = normalized.includes("left") ? "_left" : normalized.includes("right") ? "_right" : "";
  const resolved = normalized.includes("height") || normalized.includes("stature") ? "height"
    : normalized.includes("inseam") || normalized.includes("inside_leg") ? "inseam"
      : normalized.includes("foot_length") ? `foot_length${sideSuffix}`
        : normalized.includes("foot_width") ? `foot_width${sideSuffix}`
          : normalized.includes("back_to_shoulder") ? "back_to_shoulder"
            : normalized.includes("neck_to_pelvis") ? "neck_to_pelvis"
              : normalized.includes("ankle") ? `ankle${sideSuffix}`
                : normalized.includes("wrist") ? `wrist${sideSuffix}`
                  : normalized.includes("forearm") ? `forearm${sideSuffix}`
                    : normalized.includes("arm_length") || normalized === "arm" || normalized.includes("sleeve") ? `arm_length${sideSuffix}`
                      : normalized.includes("upper_arm") || normalized.includes("bicep") ? `upper_arm${sideSuffix}`
                        : normalized.includes("shoulder") ? "shoulder"
                          : normalized.includes("head") ? "head"
                            : normalized.includes("neck") ? "neck"
                              : normalized.includes("bust") || normalized.includes("chest") ? "chest"
                                : normalized.includes("waist") ? "waist"
                                  : normalized.includes("hip") ? "hip"
                                    : normalized.includes("thigh") ? `thigh${sideSuffix}`
                                      : normalized.includes("calf") ? `calf${sideSuffix}`
                                        : normalized;
...
function InteractiveBodyModel({ referenceImage, measurements = [], heightValue = null, heightUnit = "cm", focusedMeasurementKey = null }: { referenceImage: string; measurements?: Measurement[]; heightValue?: number | null; heightUnit?: "cm" | "ftin"; focusedMeasurementKey?: string | null }) {
  ...
  useEffect(() => {
    ...
    guides.traverse((object) => {
      ...
      const isFocused = focusedKey !== null && guideKeys.includes(focusedKey);
  ...
  return <div className="model-3d-viewer"><div id="model-3d-region" className="model-3d-stage" role="region" aria-label="Interactive measured body model" data-model-version="v5" aria-busy={viewerState === "loading"}><canvas ref={canvasRef} tabIndex={viewerState === "fallback" ? -1 : 0} aria-hidden={viewerState === "fallback"} aria-label="Interactive measurement-driven 3D body model" aria-describedby={`${instructionsId} ${focusStatusId}`} onKeyDown={handleCanvasKeyDown} /><figure className="model-3d-reference"><img src={referenceImage} alt="Reference body scan illustration" /><figcaption>AI reference</figcaption></figure><div className="model-3d-scale" aria-label="Model scale summary"><strong>MEASURED BODY MODEL</strong><span>{modelHeightLabel}</span><small>{measurements.length} result values mapped to the body</small></div>{viewerState === "loading" && <div className="model-3d-loading" role="status" aria-live="polite"><span className="model-3d-spinner" />Preparing measured fit mesh…</div>}{viewerState === "fallback" && <div className="model-3d-fallback" role="status"><img src={referenceImage} alt="Reference visualization for the body measurement result" /><span>Interactive 3D is unavailable on this device. Showing the reference visual.</span></div>}<span className="model-preview-badge"><Icon name="scan" size={13} /> Measured body model · interactive 3D</span><p className={cn("model-3d-focus-status", focusedMeasurement && "active")} id={focusStatusId} role="status" aria-live="polite">{focusedMeasurement ? `Focused guide: ${focusedMeasurementLabel}` : "Select a measurement row to focus its guide."}</p></div><div className="model-3d-controls" role="group" aria-label="3D model controls"><button type="button" className={autoRotate ? "active" : ""} onClick={() => setAutoRotate((value) => !value)} disabled={viewerState !== "ready" || reducedMotion} aria-pressed={autoRotate}>{autoRotate ? "Pause rotation" : "Auto rotate"}<Icon name="rotate" size={14} /></button><button type="button" onClick={() => { hasUserInteractedRef.current = true; controlsRef.current?.reset(); renderRequestRef.current?.(); }} disabled={viewerState !== "ready"}><Icon name="refresh" size={14} />Reset view</button><button type="button" onClick={zoomIn} disabled={viewerState !== "ready"}><Icon name="zoom-in" size={14} />Zoom in</button><button type="button" onClick={zoomOut} disabled={viewerState !== "ready"}><Icon name="zoom-in" size={14} />Zoom out</button><button type="button" className={showGuides ? "active" : ""} onClick={() => setShowGuides((value) => !value)} disabled={viewerState !== "ready"} aria-pressed={showGuides}><Icon name="ruler" size={14} />{showGuides ? "Hide guides" : "Show guides"}</button></div><p className="model-3d-hint" id={instructionsId}><Icon name="rotate" size={13} /> Drag to rotate · scroll or pinch to zoom · focus the model and use arrow keys, +/−, or Home</p>{reducedMotion && <p className="model-3d-motion-note" role="status">Auto-rotation is off because reduced motion is enabled.</p>}</div>;
}
...
function ModelViewer({ model, measurements = [], heightValue = null, heightUnit = "cm", focusedMeasurementKey = null }: { model: ScanBundle["bodyModel"]; measurements?: Measurement[]; heightValue?: number | null; heightUnit?: "cm" | "ftin"; focusedMeasurementKey?: string | null }) {
  ...
  return <div className="model-empty model-empty-preview model-provider-preview"><InteractiveBodyModel referenceImage={LOCAL_REFERENCE_IMAGE} measurements={measurements} heightValue={heightValue} heightUnit={heightUnit} focusedMeasurementKey={focusedMeasurementKey} /><div className="model-preview-copy"><span className="model-empty-icon"><Icon name="scan" size={29} /></span><div><p className="eyebrow">MEASUREMENT-DRIVEN PREVIEW</p><h3>Interactive 3D body model</h3><p>{providerReady ? "The fitted view stays interactive so every returned measurement can be selected and checked against the body. The provider asset remains available as a separate authorized file." : assetError || "A human-shaped reference model is proportioned from the returned measurements while a provider-specific mesh is unavailable."}</p><Badge tone={providerReady ? "success" : "teal"}>{providerReady ? "Provider asset + measured guides" : "Result values mapped to guides"}</Badge>{providerReady && assetUrl && <a className="button button-secondary model-provider-link" href={assetUrl} target="_blank" rel="noreferrer">Open provider model <Icon name="external" size={15} /></a>}{providerReady && !assetUrl && !assetError && <small className="model-provider-loading">Loading private provider asset…</small>}</div></div></div>;
}
