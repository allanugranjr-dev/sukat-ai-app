...
function CustomerDashboard({ profile, onNavigate }: { profile: Profile; onNavigate: (page: string) => void }) {
  ...
  return <div className="page-stack"><SectionHeader eyebrow={`CUSTOMER WORKROOM · ${formatDate(new Date())}`} title={`Good morning, ${profile.first_name}.`} description="Keep your measurement record clear, private, and ready for its next useful step." action={<Button variant="secondary" icon="scan" onClick={() => onNavigate("scan")}>Start a scan</Button>} />{loading ? <LoadingState /> : error ? <ErrorState message={error} onRetry={() => { scansState.reload(); ordersState.reload(); }} /> : <><section className="welcome-banner"><div><Badge tone={verifiedScans.length > 0 ? "success" : "teal"} dot>{verifiedScans.length > 0 ? "MEASUREMENT RECORD ACTIVE" : "ACCOUNT READY"}</Badge><h2>{verifiedScans.length > 0 ? "Your verified fit record is ready." : "Start with a clearer fit record."}</h2><p>{activeScan ? `Your current scan is ${scanStatusLabel(activeScan.status).toLowerCase()}.` : "Create a guided scan when you are ready. Results appear after local demo processing or a configured provider returns valid data."}</p><div className="welcome-actions"><Button onClick={() => onNavigate(activeScan ? "scan" : "scan")} icon="scan">{activeScan ? "Continue scan" : "Start a scan"}</Button>{verifiedScans.length > 0 && <Button variant="secondary" onClick={() => onNavigate("orders")} icon="bag">Start an order</Button>}</div></div><div className="welcome-orbit"><div className="orbit-ring ring-a" /><div className="orbit-ring ring-b" /><span><Icon name="ruler" size={27} /></span></div></section><div className="stats-grid four-stats"><StatCard icon="scan" label="Scans" value={String(scans.length)} detail={activeScan ? scanStatusLabel(activeScan.status) : "No active scan"} /><StatCard icon="ruler" label="Verified sets" value={String(verifiedScans.length)} detail={verifiedScans.length ? "Ready to share" : "Not available yet"} /><StatCard icon="bag" label="Orders" value={String(orders.length)} detail={orders.length ? "From your account" : "No orders yet"} /><StatCard icon="calendar" label="Fittings" value="—" detail="No fitting requests" /></div><div className="dashboard-grid"><Panel className="latest-measurement"><div className="panel-heading"><div><p className="eyebrow">LATEST ACTIVITY</p><h2>{activeScan ? "Current scan" : "No measurements yet"}</h2></div>{activeScan && <StatusBadge status={activeScan.status} />}</div>{activeScan ? <div className="status-card"><span className="status-card-icon"><Icon name="scan" size={22} /></span><div><strong>Scan created {formatDate(activeScan.created_at)}</strong><p>{activeScan.status === "processing_queued" || activeScan.status === "processing" ? "Your uploaded views are waiting for local processing or a validated provider result." : "Continue the guided flow to add or review your views."}</p></div><Button variant="ghost" onClick={() => onNavigate("scan")} icon="arrow-right">Open</Button></div> : <DataState icon="ruler" title="No measurements yet" body="Start a scan to create your first private measurement record." action={<Button onClick={() => onNavigate("scan")} icon="scan">Start a scan</Button>} />}</Panel><Panel className="scan-prompt"><p className="eyebrow">HOW IT WORKS</p><h2>A guided path from capture to review.</h2><div className="mini-steps"><span><i>01</i><b>Capture</b><small>Front, side, back</small></span><span><i>02</i><b>Validate</b><small>Provider result</small></span><span><i>03</i><b>Review</b><small>Dressmaker check</small></span></div><button className="text-button" onClick={() => onNavigate("scan")}>Open scan guide <Icon name="arrow-right" size={15} /></button></Panel></div></>}</div>;
}

function StatCard({ icon, label, value, detail }: { icon: IconName; label: string; value: string; detail: string }) {
  return <div className="stat-card"><span className="stat-icon"><Icon name={icon} size={19} /></span><span className="stat-copy"><small>{label}</small><strong>{value}</strong><em>{detail}</em></span></div>;
}
...
function ScanCapture({ captures, captureIndex, setCaptureIndex, cameraOn, videoRef, uploading, onStartCamera, onStopCamera, onCapture, onChooseFile, onRemove, onContinue, busy }: { captures: CaptureSlot[]; captureIndex: number; setCaptureIndex: (index: number) => void; cameraOn: boolean; videoRef: React.RefObject<HTMLVideoElement | null>; uploading: boolean; onStartCamera: () => void; onStopCamera: () => void; onCapture: () => void; onChooseFile: (event: ChangeEvent<HTMLInputElement>) => void; onRemove: (index: number) => Promise<void>; onContinue: () => void; busy: boolean }) {
  ...
  return <div className="capture-layout"><Panel className="capture-stage-card"><div className="capture-stage-heading"><div><p className="eyebrow">STEP 03 · CAPTURE</p><h2>{current?.label ?? "Front"} view</h2><p>Keep your full body in frame and follow the on-screen guide.</p></div><Badge tone={current?.captured ? "success" : "teal"} dot>{current?.captured ? "UPLOADED" : "READY"}</Badge></div><div className={cn("capture-stage", currentUrl && "has-capture")}>{cameraOn && <video ref={videoRef} autoPlay muted playsInline className="capture-video" />}{!cameraOn && currentUrl && <img className="capture-preview" src={currentUrl} alt={`${current?.label} scan view`} />}{!cameraOn && !currentUrl && <div className="capture-empty"><span><Icon name="camera" size={31} /></span><strong>Camera or upload</strong><small>Your selected view will appear here after a successful upload.</small></div>}{cameraOn && <div className="capture-guide"><span /><span /><span /></div>}</div><div className="capture-controls"><div className="capture-progress"><p className="eyebrow">VIEWS</p><div>{captures.map((slot, index) => <button key={slot.key} className={cn(index === captureIndex && "current", slot.captured && "done")} onClick={() => setCaptureIndex(index)}><i>{slot.captured ? <Icon name="check" size={12} /> : index + 1}</i><small>{slot.label}</small></button>)}</div></div><div className="capture-actions">{cameraOn ? <><Button variant="secondary" onClick={onStopCamera} icon="x">Stop camera</Button><Button onClick={onCapture} disabled={uploading} icon="camera">{uploading ? "Uploading…" : "Capture frame"}</Button></> : <><label className="button button-secondary"><input type="file" accept="image/jpeg,image/png,image/webp" onChange={onChooseFile} hidden /> <Icon name="upload" size={15} /> {uploading ? "Uploading…" : "Upload image"}</label><Button variant="ghost" onClick={onStartCamera} disabled={uploading} icon="camera">Use camera</Button></>}</div></div></Panel><div className="capture-side"><Panel className="capture-next"><p className="eyebrow">NEXT STEP</p><h3>{captures.every((slot) => slot.captured) ? "All three views are ready." : `Add the ${captures.find((slot) => !slot.captured)?.label.toLowerCase() ?? "next"} view.`}</h3><p>{captures.every((slot) => slot.captured) ? "Submit the scan to the processing queue when you are satisfied with the uploads." : "You can move between views at any time and replace an uploaded image."}</p><Button onClick={onContinue} disabled={busy || uploading || captures.some((slot) => !slot.captured)} icon="arrow-right">{busy ? "Submitting…" : "Submit scan"}</Button></Panel><Panel className="capture-quality"><p className="eyebrow">UPLOAD REQUIREMENTS</p><div className="quality-list"><span><Icon name="check" size={14} /> JPG, PNG, or WebP</span><span><Icon name="check" size={14} /> Maximum 10 MB each</span><span><Icon name="check" size={14} /> Private signed access</span></div><div className="uploaded-list">{captures.map((slot, index) => <div key={slot.key}><span className={slot.captured ? "uploaded" : "not-uploaded"}><Icon name={slot.captured ? "check" : "clock"} size={12} /></span><span><strong>{slot.label}</strong><small>{slot.captured ? "Uploaded" : "Waiting"}</small></span>{slot.captured && <button className="icon-button" aria-label={`Remove ${slot.label} view`} onClick={() => void onRemove(index)}><Icon name="x" size={14} /></button>}</div>)}</div></Panel></div></div>;
}
...
function MeasurementHighlights({ measurements }: { measurements: Measurement[] }) {
  const highlights = highlightMeasurements(measurements);
  if (highlights.length === 0) return null;
  return <div className="measurement-highlights" aria-label="Key measurements">{highlights.map((measurement) => <article className="measurement-highlight" key={measurement.id}><span>{displayMeasurementKey(measurement.key)}</span><strong>{displayMeasurementValue(measurement)}</strong><small>{measurement.confidence === null ? "Confidence not reported" : `${measurement.confidence.toFixed(0)}% confidence`}</small></article>)}</div>;
}
...
function InteractiveBodyModel({ generatedImage, measurements = [], heightValue = null, heightUnit = "cm" }: { generatedImage: string; measurements?: Measurement[]; heightValue?: number | null; heightUnit?: "cm" | "ftin" }) {
  ...
  const measurementSignature = measurements.map((measurement) => `${measurement.key}:${measurement.value}`).join("|");
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      try {
        ...
        scene.add(grid);
        const bodyScene = createBodyModelScene(three, measurements, heightValue, heightUnit);
...
function ModelViewer({ model, measurements = [], heightValue = null, heightUnit = "cm" }: { model: ScanBundle["bodyModel"]; measurements?: Measurement[]; heightValue?: number | null; heightUnit?: "cm" | "ftin" }) {
  ...
  if (localPreview) {
    const generatedImage = localPreviewPath(localPreview.generated_image) ?? localPreviewPath(localPreview.poster) ?? publicAssetPath("/media/3d-body-scan-generated.png");
    return <div className="model-empty model-empty-preview model-local-preview"><InteractiveBodyModel generatedImage={generatedImage} measurements={measurements} heightValue={heightValue} heightUnit={heightUnit} /><div className="model-preview-copy"><span className="model-empty-icon"><Icon name="scan" size={29} /></span><div><p className="eyebrow">GENERATED MODEL VISUAL</p><h3>Interactive 3D visualization ready</h3><p>The generated image reference now drives an interactive 3D mannequin. Drag to rotate, zoom to inspect, and toggle measurement guides. Local measurements remain illustrative estimates and must be checked by a dressmaker before tailoring.</p><Badge tone="warning">Illustrative model · verify before tailoring</Badge></div></div></div>;
  }
  if (!model || model.status !== "ready" || !model.model_url_or_path || assetError) return <div className="model-empty model-empty-preview"><InteractiveBodyModel generatedImage={publicAssetPath("/media/3d-body-scan-generated.png")} measurements={measurements} heightValue={heightValue} heightUnit={heightUnit} /><div className="model-preview-copy"><span className="model-empty-icon"><Icon name="scan" size={29} /></span><div><p className="eyebrow">REFERENCE MODEL VISUAL</p><h3>Interactive 3D preview</h3><p>{assetError || "An interactive reference model is available while the reconstruction provider prepares a personalized body asset."}</p><Badge tone="neutral">Preview only · no measurements</Badge></div></div></div>;
...
function ProfilePage({ profile, onProfileChange }: { profile: Profile; onProfileChange: (profile: Profile) => void }) {
  ...
  const save = async (event: FormEvent) => {
    ...
    try { const updated = await updateProfile(profile.id, { first_name: firstName, last_name: lastName, unit_system: unit }); onProfileChange(updated); setNotice("Profile updated."); } catch (reason: unknown) { setError(readableError(reason)); } finally { setBusy(false); }
  };
  ...
  return <div className="page-stack"><SectionHeader eyebrow="ACCOUNT" title="Profile" description="Keep your account details and measurement display preference up to date." /><div className="profile-layout"><Panel className="profile-card"><div className="profile-identity"><Avatar profile={profile} size="lg" /><div><h2>{displayName(profile)}</h2><p>{profile.email}</p><Badge tone={profile.role === "admin" ? "blue" : profile.role === "dressmaker" ? "warning" : "teal"}>{profile.role}</Badge></div></div><form className="profile-fields" onSubmit={save}><Field label="First name" value={firstName} onChange={setFirstName} /><Field label="Last name" value={lastName} onChange={setLastName} /><div className="field"><label htmlFor="profile-unit">Measurement display</label><select id="profile-unit" value={unit} onChange={(event) => setUnit(event.target.value as "cm" | "ftin")}><option value="cm">Centimetres</option><option value="ftin">Feet / inches</option></select></div><div className="field"><label>Email address</label><input value={profile.email} disabled /></div>{notice && <div className="form-notice"><Icon name="check" size={15} /> {notice}</div>}{error && <InlineError message={error} />}<Button type="submit" disabled={busy} icon="check">{busy ? "Saving…" : "Save profile"}</Button></form></Panel><Panel className="settings-card"><p className="eyebrow">SECURITY</p><h2>Account access</h2><p>Your email is managed by Supabase Auth. Use a secure reset link when you need to change your password.</p><div className="provider-row"><span><Icon name="mail" size={16} /> Verified email</span><Badge tone="success" dot>Auth managed</Badge></div><Button variant="secondary" onClick={() => void passwordReset()} icon="lock">Send password reset link</Button><div className="data-actions"><button onClick={() => void signOut()}><Icon name="logout" size={15} /> Sign out of this account</button></div></Panel></div></div>;
}
...
function AdminInvitations({ profile }: { profile: Profile }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setBusy(true);
    try { const result = await inviteDressmaker({ email, organizationId, redirectTo: `${window.location.origin}/?invite=` }); setNotice("Invitation created. Share the secure link with the invited dressmaker."); setInviteUrl(result.inviteUrl ?? ""); setEmail(""); invitationsState.reload(); } catch (reason: unknown) { setError(readableError(reason)); } finally { setBusy(false); }
  };
